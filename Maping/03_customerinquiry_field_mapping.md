...
| 14 | Inquiry form (CI) | Choosing **Type** (by the user) sets the client defaults: Status **DSR CLOSED** for DSR - NO ACTION NEEDED, otherwise **OPEN**; Purpose **1st DSR Visit To Be Done …** for the DSR types when Purpose is empty. Opening an existing inquiry does not change anything | Behaviour | `assets/scripts/inquiry.js` (`change.fupdef` on `#inqfup_type`, only when select2 sends `e.val` (user choice)) | `inqcs.status`, `followup.purpose` |
| 15 | Inquiry form (CI) | **Required fields as in eBizWiz** (standard `required` class + red *): Type; Purpose, Priority, Date; Action Taken, Action Taken Date, Person Contacted; Next Action To Be Taken, Action To Be Taken Date, Priority, Party Contact, Sales Person. Remarks stay optional. A section hidden by the Type is not validated (standard validator ignores hidden fields) | Validation | `inquiry/default.asp` | — |
| 16 | Inquiry form (CI) | Fields of a section hidden by the Type (Action Taken / Next Action) are not sent on save, so switching the Type does not create a hidden follow-up | Behaviour | `assets/scripts/inquiry.js` (inquiry_form submit, `fupDrop`) | `followup` |
| 17 | Inquiry form (CI) edit | Type is also filled from the inquiry itself (`inqcs.types`) when the inquiry has no follow-up (23 migrated inquiries) | Behaviour | `assets/scripts/inquiry.js` (edit-fill, `aInquiry[S + 7]`) | `inqcs.types` |
